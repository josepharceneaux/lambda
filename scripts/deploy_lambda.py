...
def make_deployment_dir(deployment_parent_directory='./deployments'):
    ...
    if not deployment_parent_directory or not os.path.isdir(deployment_parent_directory):
        ...
        return "{0}/{1}".format(deployment_parent_directory, deployment_name)
    ...
    # Not sure we really need a directory for every version, we could just have separate zip files
...
def copy_virtual_env_libs(deployment_dir, lib64=False):
    ...
    if sys.real_prefix:
        ...
        venv_dir = sys.real_prefix
    else:
        # Otherwise use the system environment
        venv_dir = sys.prefix

    package_dir = "{}/lib/python3.7/site-packages".format(venv_dir)
    lib64_dir = "{}/lib64/python2.7/site-packages/".format(venv_dir)
    print("Copying packages from environment {}".format(package_dir))

    if os.path.exists(venv_dir):
        lib64_dir = "{}/lib64/python2.7/site-packages/".format(venv_folder)
        cmd = "cp -r {0}* {1}".format(lib_dir, deployment_dir)
        subprocess.call(cmd, shell=True)
        if lib64:
            cmd = "cp -r {0}* {1}".format(lib64_dir, deployment_dir)
            subprocess.call(cmd, shell=True)
    else:
